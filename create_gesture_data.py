...
path = "E:\PyCharm\PycharmProjects\SC-Project\Different-Hand-Gesture-Recognition-Using-CNN\Traindata"

traindata = glob.glob(path + "\*")
noOfGestures = len(traindata)
print("Number of Gestures already available: ", noOfGestures)

path = path + "\Gesture_" + str(noOfGestures)
    ...
    # Time to configure the hand specifically into the ROI...
        ...
        if hand is not None:
            ...
            if num_imgs_taken <= 300:
                ...
                cv2.imwrite(path + "\\" + "gest" + str(noOfGestures) + "_" + str(num_imgs_taken+1) + '.jpg', thresholded)
            else:
                break
            num_imgs_taken += 1
        else:
